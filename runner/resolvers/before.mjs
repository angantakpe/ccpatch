// runner/resolvers/before.mjs
// Resolver for @At BEFORE — immediately before a string literal.
...
/**
 * Resolve a BEFORE selector — zero-length insertion point immediately before
 ...
 */
export function resolveBefore(target, code, _opts = {}) {
  const lit = target?.literal;
  if (typeof lit !== 'string' || lit.length === 0) {
    return { ok: false, error: 'BEFORE: target.literal (non-empty string) required' };
  }
  const occurrence = target.occurrence ?? 1;
  if (!Number.isInteger(occurrence) || occurrence < 1) {
    return { ok: false, error: 'BEFORE: target.occurrence must be a positive integer' };
  }
  let idx = -1;
  let from = 0;
  ...
  if (idx === -1) {
    return {
      ok: false,
      error: `BEFORE: literal not found (${JSON.stringify(lit.slice(0, 40))})`,
  ...
  return {
    ...
    sites: [{
      ...
      kind: 'BEFORE',
      label: `BEFORE ${JSON.stringify(lit.slice(0, 40))}`,
    }],
  };
}
