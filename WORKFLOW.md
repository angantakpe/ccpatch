...
tracker:
  kind: linear
  provider:
    project_slug: "ccpatch-66fefe696a37"
  required_labels:
    - "repo:ccpatch"
  active_states:
    - Todo
...
polling:
  interval_ms: 15000
workspace:
  root: ~/symphony-workspaces/ccpatch
hooks:
  after_create: |
    git clone --depth 1 https://github.com/angantakpe/ccpatch.git .
...
`~/symphony-workspaces/ccpatch`). Treat every other path on this machine as off-limits, including
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/home/dev-user/symphony-workspaces/ccpatch/COD-816@7bdde33bc`
...
5.  Run validation/tests required for the scope.
    - Mandatory gate: run `npm run lint` and `npm run test:all` (the same checks CI runs) and treat any failure as incomplete work.
    ...
    - Prefer a targeted proof that directly demonstrates the behavior you changed
      (for example a focused `npm run test:patch` / `npm run test:boot` run).
...
- `npm run lint` and `npm run test:all` are green for the latest commit.
